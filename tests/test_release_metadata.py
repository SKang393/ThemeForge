...
class ReleaseMetadataTests(unittest.TestCase):
    def test_version_metadata_is_1_3_0(self):
        pyproject = tomllib.loads((ROOT / "pyproject.toml").read_text(encoding="utf-8"))
        version_file = (ROOT / "VERSION").read_text(encoding="utf-8").strip()
        init_version = themeforge.__version__

        self.assertEqual(pyproject["project"]["version"], "1.3.0")
        self.assertEqual(version_file, "1.3.0")
        self.assertEqual(init_version, "1.3.0")

    def test_readme_and_build_defaults_reference_1_3_0(self):
