...
class ReleaseMetadataTests(unittest.TestCase):
    def test_version_metadata_is_1_4_1(self):
        pyproject = tomllib.loads((ROOT / "pyproject.toml").read_text(encoding="utf-8"))
        version_file = (ROOT / "VERSION").read_text(encoding="utf-8").strip()
        init_version = themeforge.__version__

        self.assertEqual(pyproject["project"]["version"], "1.4.1")
        self.assertEqual(version_file, "1.4.1")
        self.assertEqual(init_version, "1.4.1")

    def test_readme_and_build_defaults_reference_1_4_1(self):
        readme = (ROOT / "README.md").read_text(encoding="utf-8")
        build_script = (ROOT / "scripts" / "build_windows_portable.ps1").read_text(encoding="utf-8")

        self.assertIn("v1.4.1", readme)
        self.assertRegex(build_script, re.compile(r'\[string\]\$Version = "1\.4\.1"'))
    ...
    def test_release_note_and_portable_nlp_scope_are_present(self):
        readme = (ROOT / "README.md").read_text(encoding="utf-8")
        release_note = (ROOT / "release_notes" / "v1.4.1.md").read_text(encoding="utf-8")
        build_script = (ROOT / "scripts" / "build_windows_portable.ps1").read_text(encoding="utf-8")

        self.assertIn("# ThemeForge v1.4.1", release_note)
